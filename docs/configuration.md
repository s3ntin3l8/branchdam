...
| `readOnly` | bool | `false` | Enforced by `storage.Guard.CheckWrite`, which refuses any write against a read-only location before any syscall. Tier 3 is read-only unless `readOnly: false` is configured; the `:ro` mount remains a defense-in-depth default. |
