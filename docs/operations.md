...
| `/api/v1/agent/*` returns `503` | No pairing service wired (nil `LookupKey` gate) | Wire a `Pairing.KeyLookup` implementation, or pair a device via `POST /api/v1/agent/handshake/pair` and retry with its per-device key. |
| `/api/v1/agent/*` returns `401` with a key set | Key doesn't match any paired device, or `X-API-Key` header not being forwarded by Traefik | Confirm the key in your request matches the paired device's key exactly; confirm the agent router's rule matches the request path. |
