...
| `/api/v1/agent/*` returns `503` | No pairing service configured, or the presented key has no matching paired device | Pair a device via `POST /api/v1/agent/handshake/pair` and retry with its per-device key. |
