...
func TestLoadGuardResolvesRootPaths(t *testing.T) {
	...
	guard, err := LoadGuard(context.Background(), &fakeLister{rows: []StorageLocationRow{
...
func TestLoadGuardFailsLoudlyOnMissingRoot(t *testing.T) {
	_, err := LoadGuard(context.Background(), &fakeLister{rows: []StorageLocationRow{
		{ID: 1, Name: "archive", RootPath: "/does/not/exist/anywhere", Tier: "TIER3_MASTER_ARCHIVE", ReadOnly: true},
	}})
	if err == nil {
		t.Fatal("LoadGuard with a nonexistent root_path succeeded, want an error")
	}
}
