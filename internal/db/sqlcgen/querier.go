...
type Querier interface {
	...
	// Atomic single-use consume. The handler treats a zero-row result
	// (token already used, or expired) as 404 -- no enumeration of
	// which one.
	...
	// Rotates the password_hash. Used by password-reset/confirm and
	// the admin reset endpoint. password_hash is a sql.NullString
	// because forward-jit users have NULL (CHECK-enforced) -- callers
	// that operate on local users can pass a valid string.
	...
	// Confirm-time lookup: narrow the active-token set to the row
	// whose hash matches. Returns sql.ErrNoRows on miss.
	...
	// Pending-reset tokens for one user. Used by the admin "pending
	// resets" panel in PR #408.
	ListActivePasswordResetTokens(ctx context.Context, arg ListActivePasswordResetTokensParams) ([]PasswordResetToken, error)
	// Pending-reset tokens across all users, paginated. Same consumer.
	...
	// Idempotent admin-revoke of an outstanding reset token. Used by
	// the "delete" button in the pending-resets panel.
