...
// Execute purges every candidate: re-verifies eligibility, then
// storage.Guard.Remove (so a read-only tier or a symlink escape is refused
// before any DB write -- resolved and defeated by Guard.CheckWrite's own
// canonicalize step, the same defense TestSymlinkEscapeRefused proves for
// every other Guard caller), then MarkNodeMissing -- all inside the SAME
// transaction as the re-verify, so the safety check and the write it
// gates can never observe different states. Rows are never deleted,
// matching this repo's invariant -- a purged node lands in MISSING, the
// same state a vanished file gets; TouchMediaNode reactivates it if the
// cache is later regenerated at the same path, and RebaseMissingNodePath
// can adopt its fast_hash if identical content reappears elsewhere. Both
// are the desired semantics for a cache file.
//
...
// ErrNoLongerEligible if it no longer appears. This closes the TOCTOU
// between a dry-run response and a later Execute call: without it, a
// Tier-3 master going MISSING (or its full_hash getting invalidated)
