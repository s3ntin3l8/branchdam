...
func TestPickWinningParent(t *testing.T) {
	...
	t.Run("manual edge beats older equal-confidence autogenerated edge", func(t *testing.T) {
		edges := []sqlcgen.MediaEdge{
			{ID: 1, ReviewState: "AUTO_ACCEPTED", Tier: 1, RelationshipType: "DERIVED_FROM", Confidence: 1.00, Resolver: "sidecar"},
			{ID: 10, ReviewState: "CONFIRMED", Tier: 1, RelationshipType: "DERIVED_FROM", Confidence: 1.00, Resolver: "manual"},
		}
		got := PickWinningParent(edges)
		if got == nil || got.ID != 10 {
			t.Fatalf("PickWinningParent = %+v, want manual edge ID 10", got)
		}
	})

	t.Run("newer manual edge beats older manual edge", func(t *testing.T) {
		edges := []sqlcgen.MediaEdge{
			{ID: 10, ReviewState: "CONFIRMED", Tier: 1, RelationshipType: "DERIVED_FROM", Confidence: 1.00, Resolver: "manual"},
			{ID: 15, ReviewState: "CONFIRMED", Tier: 1, RelationshipType: "DERIVED_FROM", Confidence: 1.00, Resolver: "manual"},
		}
		got := PickWinningParent(edges)
		if got == nil || got.ID != 15 {
			t.Fatalf("PickWinningParent = %+v, want newer manual edge ID 15", got)
		}
	})
}
