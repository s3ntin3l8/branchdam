...
// never prevent inheriting from the valid one.
//
// Selection ranking:
//  1. Higher confidence wins.
//  2. On equal confidence, manual edges (resolver == "manual") take precedence
//     over autogenerated edges (e.g. sidecar).
//  3. On equal confidence between manual edges, the most recently created edge
//     (higher ID) wins, making a newly-created manual parent authoritative.
//  4. On equal confidence between autogenerated edges, the lowest edge ID wins
...
func PickWinningParent(edges []sqlcgen.MediaEdge) *sqlcgen.MediaEdge {
	...
	for i := range edges {
		...
		if best == nil || isBetterParent(e, best) {
			best = e
		}
	}
	return best
...
func isBetterParent(e, best *sqlcgen.MediaEdge) bool {
	...
	// Equal confidence:
	eManual := e.Resolver == "manual"
	bestManual := best.Resolver == "manual"
	if eManual != bestManual {
		return eManual
	}
	if eManual && bestManual {
		return e.ID > best.ID
