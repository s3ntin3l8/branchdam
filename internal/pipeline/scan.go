...
type ScanDeps struct {
	...
	// pass through. Used to distinguish a shutdown-interrupted scan
	// (terminalized CANCELLED) from ordinary backpressure or completed scans
	// with failures (terminalized COMPLETED with a nonzero files_failed) --
...
// ErrScanAlreadyRunning is returned by createScanJob (and so by RunScan and
// RunSweep) when a job of the same kind is already RUNNING for the same
// storage location (#163, widened by #226). Two concurrent POSTs to
// /api/v1/scan for one location otherwise each walk it independently;
// workers.Pool.Submit dedups by file path, so the second scan's submissions
// are silently refused for every file, leaving a confusing large
// files_failed count with no indication why. Rejecting the second scan
// outright is clearer and fail-safe: ReconcileOrphanedScanJobs already flips
// any stale RUNNING row to FAILED at startup, so a crashed process can never
// permanently strand a location behind this guard.
...
// already-running check covers both kind == "FULL_SCAN" and kind ==
// "INCREMENTAL" -- WATCH jobs are excluded (never passed here) since
// they're already singleton-per-location via WatcherSupervisor.Start's
// sync.Once and are long-lived by design. INCREMENTAL was added to the
// guard by #226: SweeperSupervisor's own INCREMENTAL passes stay serialized
// by its own ticker loop regardless (so this check is a permanent no-op on
// that path), but #226 also lets an HTTP request create an INCREMENTAL job
// directly (POST /api/v1/scan {differential:true} against a Tier-3
...
func createScanJob(ctx context.Context, deps ScanDeps, location storage.Location, kind string) (sqlcgen.ScanJob, error) {
	...
	err := deps.DB.InTx(ctx, func(q *sqlcgen.Queries) error {
		if kind == "FULL_SCAN" || kind == "INCREMENTAL" {
			...
			// count.
			running, err := q.CountRunningScansForLocationByKind(ctx, sqlcgen.CountRunningScansForLocationByKindParams{
...
func runScan(ctx context.Context, deps ScanDeps, location storage.Location, jobID, startedAt int64, differential bool) {
	...
	// interrupted is set when work is interrupted or refused due to shutdown:
	// - OnAbandon (jobs queued when the pool's context was cancelled)
	// - jobCtx.Done() inside Run (pool context cancelled while sending result)
	// - Submit refused while isClosed(deps.Shutdown) is true
	// - processFile failing while jobCtx.Err() != nil.
	// It is NOT set on ordinary submit backpressure (queue full / duplicate key
	// when shutdown has not begun).
	...
	// #99 / #353: a scan that lost work to a mid-shutdown pool close is
	// CANCELLED, not COMPLETED -- mirroring watchLocation's own
	// CANCELLED-by-default convention (watcher.go) for a clean-shutdown
	// termination, as opposed to FAILED for a genuine error (the walkErr
	// branch above, which already returned before this point).
	//
	// Audit finding G6 (#345 / #353): we distinguish "shutdown before work
	// finished" (in-flight work or submissions were cancelled/aborted by
	// shutdown, tracked by interrupted) from "work completed with failures"
	// (all batches drained and committed, filesFailed > 0 due to genuine
	// errors or prior backpressure). If no work was interrupted by shutdown,
	// the scan terminalizes as COMPLETED even if shutdown was signaled
	// after all batches finished.
	//
	// The three-state contract this produces:
	//   CANCELLED  -- clean shutdown; work in-flight was interrupted before completion.
